...
client.on("ready", () => {
    ...
    client.user.setActivity({ name: "Overdose#0111 !", type: "STREAMING", url: "https://www.twitch.tv/yourragegaming" })
    Main();
});


function Main() {
    console.log("\tMass DM:\n\n\tOptions:\n    [1] Normal mode dumbass nigga\n    [2] Timeout Mode (Avoids Flagging)\n");
    readline.question("[?] Choose Option: ", answer => {
        switch (answer) {
            case "1":
                readline.question("\n[!] Enter Guild ID: ", response => {
                    ScrapeUsers(response).then(() => {
                        console.log(greenBright("Warning: Sending message to user in second."));
                        setTimeout(() => {
                                MassDMNormal(null, message).catch((err) => {
                                    console.log(err)
                                    setTimeout(() => {
                                        console.log(yellow("Warning: Fucking yo bitch wait nigga."));
                                    }, 1000);
                                    setTimeout(() => {
                                        process.exit(1);
                                    }, 2000);
